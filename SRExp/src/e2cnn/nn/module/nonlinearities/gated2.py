...
class GatedNonLinearity2(EquivariantModule):
    ...
    def __init__(self,
        ...
        for s, contiguous in self._contiguous.items():
            if contiguous:
                ...
                _input_indices[s] = mindspore.Tensor([min(_input_indices[s]), max(_input_indices[s])+1], dtype=mindspore.int64)
            else:
                # otherwise, transform the list of indices into a tensor
                _input_indices[s] = mindspore.Tensor(_input_indices[s], dtype=mindspore.int64)
        ...
        self.bias = mindspore.Parameter(mindspore.ops.StandardNormal(1, ngates, 1, 1, dtype=mindspore.float32), requires_grad=True)

    def construct(self, gates: GeometricTensor, input: GeometricTensor) -> GeometricTensor:
        r"""
    ...
    def check_equivariance(self, atol: float = 1e-6, rtol: float = 1e-5) -> List[Tuple[Any, float]]:
        ...
        x = [stdnormal((3, c[i], 10, 10)) for i in range(2)]
