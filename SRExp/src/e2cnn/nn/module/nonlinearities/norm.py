...
class NormNonLinearity(EquivariantModule):
    ...
    def __init__(self, in_type: FieldType, function: str = 'n_relu', bias: bool = True):
        ...
        if function == 'n_relu':
            self._function = mindspore.ops.Relu()
        elif function == 'n_sigmoid':
            self._function = mindspore.ops.Sigmoid()
        ...
        for s, contiguous in self._contiguous.items():
            if contiguous:
                ...
                _indices[s] = mindspore.Tensor([min(_indices[s]), max(_indices[s])+1], dtype=int64)
            else:
                # otherwise, transform the list of indices into a tensor
                _indices[s] = mindspore.Tensor(_indices[s], dtype=int64)
        ...
        if bias:
            ...
            self.log_bias = mindspore.Parameter(zeros((1, len(self.in_type), 1, 1), mindspore.float32), requires_grad=True)
        else:
            self.log_bias = None
        ...
        self.eps = mindspore.Parameter(mindspore.Tensor(1e-10), requires_grad=False)
    ...
    def construct(self, input: GeometricTensor) -> GeometricTensor:
        ...
        multipliers = mindspore.numpy.empty_like(input)
        ...
        if self.log_bias is not None:
            ...
            biases = mindspore.ops.Exp(self.log_bias)
        ...
        for s in self._order:
            ...
            _, output = mindspore.ops.ArgMaxWithValue(self.eps)(norms)
            m = new_norms / output
            m[norms <= self.eps] = 0.
