    ...
    def __init__(self, in_type: FieldType, function: str = 'n_relu', bias: bool = True):
        ...
        for id, contiguous in self._contiguous.items():
            if contiguous:
                ...
                _indices[id] = mindspore.Tensor([min(_indices[id]), max(_indices[id])+1], dtype=int64)
            else:
                # otherwise, transform the list of indices into a tensor
                _indices[id] = mindspore.Tensor(_indices[id], dtype=int64)
        ...
        if bias:
            ...
            self.log_bias = mindspore.Parameter(zeros(len(self.in_type), mindspore.float32), requires_grad=True)
        else:
            self.log_bias = None
