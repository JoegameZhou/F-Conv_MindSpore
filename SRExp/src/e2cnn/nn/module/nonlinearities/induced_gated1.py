...
class InducedGatedNonLinearity1(EquivariantModule):
    ...
    def __init__(self,
        ...
        for t, contiguous in self._contiguous.items():
            if contiguous:
                ...
                _input_indices[t] = mindspore.Tensor([min(_input_indices[t]), max(_input_indices[t]) + 1], dtype=mindspore.int64)
                if t != GATES_ID or not self.drop_gates:
                    _output_indices[t] = mindspore.Tensor([min(_output_indices[t]), max(_output_indices[t]) + 1], dtype=mindspore.int64)
            else:
                # otherwise, transform the list of indices into a tensor
                _input_indices[t] = mindspore.Tensor(_input_indices[t], dtype=mindspore.int64)
    
                if t != GATES_ID or not self.drop_gates:
                    _output_indices[t] = mindspore.Tensor(_output_indices[t], dtype=mindspore.int64)
        ...
        self.bias = mindspore.Parameter(stdnormal((1, ngates, 1, 1, 1)), requires_grad=True)
    ...
    def construct(self, input: GeometricTensor) -> GeometricTensor:
        ...
        # transform the gates
        gates = mindspore.ops.Sigmoid(gates.view(b, -1, self.quotient_size, h, w ) - self.bias)
