...
class NormMaxPool(EquivariantModule):
    ...
    def forward(self, input: GeometricTensor) -> GeometricTensor:
        ...
        output = mindspore.numpy.empty((b, c, ho, wo))
        ...
        for s, contiguous in self._contiguous.items():
            ...
            if contiguous:
                ...
                max_pool = mindspore.ops.MaxPool(self.kernel_size, self.stride)
                ...
                if use_pad:
                    norms = pad(norms)
                output_tensor = max_pool(norms)
                indx = output_tensor.argmax()
                ...
                output_tensor = max_pool(norms)
                indx = output_tensor.argmax()
                _, indx = mindspore.nn.MaxPool2d(norms,
                                       self.kernel_size,
                                       self.stride,
                                       self.padding,
                                       self.dilation,
