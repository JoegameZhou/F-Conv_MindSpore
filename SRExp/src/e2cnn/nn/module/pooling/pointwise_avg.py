...
class PointwiseAvgPool(EquivariantModule):
    ...
    def construct(self, input: GeometricTensor) -> GeometricTensor:
        ...
        avg_pool = mindspore.ops.AvgPool(self.kernel_size, self.stride)
        use_pad = self.padding != 0
        if isinstance(self.padding, tuple):
            assert len(self.padding) == 2
            paddings = ((0, 0), (0, 0), (self.padding[0], self.padding[0]), (self.padding[1], self.padding[1]))
        elif isinstance(self.padding, int):
            paddings = ((0, 0),) * 2 + ((self.padding, self.padding),) * 2
        else:
            raise ValueError('padding should be a tuple include 2 numbers or a int number')
        pad = mindspore.ops.Pad(paddings)
        if use_pad:
            input.tensor = pad(input.tensor)
        output = avg_pool(input.tensor)
...
class PointwiseAvgPoolAntialiased(EquivariantModule):
    ...
    def __init__(self,
        ...
        grid_x = mindspore.numpy.arange(filter_size).repeat(filter_size).view(filter_size, filter_size)
        grid_y = grid_x.t()
        grid = mindspore.ops.Stack(-1)([grid_x, grid_y])

        mean = (filter_size - 1) / 2.
        variance = sigma ** 2.

        # setting the dtype is needed, otherwise it becomes an integer tensor
        r = -mindspore.ops.ReduceSum()((grid - mean) ** 2., -1)

        # Build the gaussian kernel
        _filter = mindspore.ops.Exp()(r / (2 * variance))

        # Normalize
        _filter /= mindspore.ops.ReduceSum()(_filter)
    ...
    def construct(self, input: GeometricTensor) -> GeometricTensor:
        ...
        output = mindspore.ops.Conv2D(input.tensor, self.filter, stride=self.stride, padding=self.padding, groups=input.shape[1])
