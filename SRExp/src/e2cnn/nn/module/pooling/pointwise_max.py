...
class PointwiseMaxPool(EquivariantModule):
    ...
    def forward(self, input: GeometricTensor) -> GeometricTensor:
        ...
        output = mindspore.nn.MaxPool2d(input.tensor,#difference
                              self.kernel_size,
                              self.stride,
                              self.padding,
                              self.dilation,
    ...
    def export(self):
        ...
        return mindspore.nn.MaxPool2d(self.kernel_size, self.stride, self.padding, self.dilation).eval()#difference
...
class PointwiseMaxPoolAntialiased(PointwiseMaxPool):
    ...
    def __init__(self,
        ...
        grid_y = grid_x.t()
        grid = mindspore.ops.Stack([grid_x, grid_y], dim=-1)

        mean = (filter_size - 1) / 2.
        variance = sigma ** 2.
        
        # setting the dtype is needed, otherwise it becomes an integer tensor
        r = -mindspore.ops.ReduceSum((grid - mean) ** 2., dim=-1, dtype=torch.get_default_dtype())#difference
    ...
    def forward(self, input: GeometricTensor) -> GeometricTensor:
        ...
        output = mindspore.ops.Conv2D(output, self.filter, stride=self.stride, padding=self._pad, groups=output.shape[1])
