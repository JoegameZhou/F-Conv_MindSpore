...
class R2Upsampling(EquivariantModule):
    ...
    def construct(self, input: GeometricTensor):
        ...
        resize_bilinear = mindspore.nn.ResizeBilinear()
        if self._align_corners is None:
            output = resize_bilinear(input.tensor,
    ...
    def check_equivariance(self, atol: float = 0.1, rtol: float = 0.1):
        ...
        x = GeometricTensor(mindspore.Tensor(x, dtype=float32), self.in_type)
