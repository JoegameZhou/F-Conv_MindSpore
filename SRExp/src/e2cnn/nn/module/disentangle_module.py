...
class DisentangleModule(EquivariantModule):
    ...
    def __init__(self, in_type: FieldType):
        ...
        for r in self.in_type._unique_representations:
            ...
            _change_of_basis_matrices[r.name] = mindspore.Tensor(cob, dtype=float32)
        ...
        for repr_name, (contiguous, fields_indices, fiber_indices) in grouped_indices.items():
            ...
            fiber_indices = mindspore.Tensor(fiber_indices, dtype=int64)
    ...
    def construct(self, input: GeometricTensor) -> GeometricTensor:
        ...
        for repr_name in self._order:
            ...
            equation = "oi,bciwh->bcowh"
            einsum = mindspore.ops.Einsum(equation)
            transformed_fields = einsum((cob, input_fields)).reshape(b, -1, w, h)
