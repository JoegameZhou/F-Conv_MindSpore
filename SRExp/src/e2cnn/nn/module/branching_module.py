...
class BranchingModule(EquivariantModule):
    ...
    def __init__(self,
        ...
        for l, contiguous in self._contiguous.items():
            if contiguous:
                ...
                _input_indices[l] = mindspore.Tensor([min(_input_indices[l]), max(_input_indices[l])+1], dtype=int64)
                
            else:
                # for the others, the indices list is trasformed into a PyTorch's Tensor
                _input_indices[l] = mindspore.Tensor(_input_indices[l], dtype=int64)
    ...
    def construct(self, input: GeometricTensor) -> Dict[str, GeometricTensor]:
        r"""
