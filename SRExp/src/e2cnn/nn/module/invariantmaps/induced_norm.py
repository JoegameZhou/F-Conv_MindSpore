...
class InducedNormPool(EquivariantModule):
    ...
    def __init__(self, in_type: FieldType, **kwargs):
        ...
        for id, contiguous in self._contiguous.items():
            if contiguous:
                ...
                _in_indices[id] = mindspore.Tensor([min(_in_indices[id]), max(_in_indices[id])+1], dtype=mindspore.int64)
                _out_indices[id] = mindspore.Tensor([min(_out_indices[id]), max(_out_indices[id])+1], dtype=mindspore.int64)
            else:
                # otherwise, transform the list of indices into a tensor
                _in_indices[id] = mindspore.Tensor(_in_indices[id], dtype=mindspore.int64)
                _out_indices[id] = mindspore.Tensor(_out_indices[id], dtype=mindspore.int64)
    ...
    def construct(self, input: GeometricTensor) -> GeometricTensor:
        ...
        output = mindspore.numpy.empty(self.evaluate_output_shape(input.shape), dtype=mindspore.float32)
    ...
    def check_equivariance(self, atol: float = 1e-6, rtol: float = 1e-5) -> List[Tuple[Any, float]]:
        ...
        for el in self.space.testing_elements:
            ...
            assert torch.allclose(tensor_to_numpy1, tensor_to_numpy2, atol=atol, rtol=rtol), \
