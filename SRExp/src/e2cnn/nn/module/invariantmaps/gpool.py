...
class GroupPooling(EquivariantModule):
    ...
    def __init__(self, in_type: FieldType, **kwargs):
        ...
        for s, (contiguous, fields, idxs) in indeces.items():
            ...
            if contiguous:
                ...
                _in_indices[s] = mindspore.Tensor([min(idxs), max(idxs)+1], dtype=mindspore.int64)
                _out_indices[s] = mindspore.Tensor([min(fields), max(fields)+1], dtype=mindspore.int64)
            else:
                # otherwise, transform the list of indices into a tensor
                _in_indices[s] = mindspore.Tensor(idxs, dtype=mindspore.int64)
                _out_indices[s] = mindspore.Tensor(fields, dtype=mindspore.int64)
    ...
    def construct(self, input: GeometricTensor) -> GeometricTensor:
        ...
        for s, contiguous in self._contiguous.items():
            ...
            _, max_activations = mindspore.ops.ArgMaxWithValue(axis=2)(fm)
