...
def dropout_field(input: mindspore.Tensor, p: float, training: bool, inplace: bool):
    ...
    if training:
        ...
        if input.device == 'cpu':
            mask = mindspore.Tensor(*shape, dtype=mindspore.float32)
        else:
            device = input.device
            mask = mindspore.Tensor(*shape, dtype=mindspore.float32)
        
        mask = mask.uniform_() > p
        mask = mask.to(mindspore.float32)
