    ...
    def __init__(self,
        ...
        for s, contiguous in self._contiguous.items():
            if contiguous:
                ...
                _indices[s] = mindspore.Tensor([min(_indices[s]), max(_indices[s])+1], dtype=mindspore.float32)
            else:
                # otherwise, transform the list of indices into a tensor
                _indices[s] = mindspore.Tensor(_indices[s], dtype=mindspore.float32)
                
            # register the indices tensors as parameters of this module
            self.register_buffer(f'{s}_indices', _indices[s])
            
            ones = mindspore.ops.Ones()
            running_var = ones((1, self._nfields[s], 1, 1, 1), mindspore.float32)
            self.register_buffer(f'{s}_running_var', running_var)
            
            if self.affine:
                weight = Parameter(ones((1, self._nfields[s], 1, 1, 1), mindspore.float32), requires_grad=True)
