...
class GNormBatchNorm(EquivariantModule):
    ...
    def __init__(self,
        ...
        for name, contiguous in self._contiguous.items():
            ...
            if contiguous:
                ...
                _indices[name] = mindspore.Tensor(_indices[name],mindspore.int64)
        ...
        for r in self.in_type._unique_representations:
            ...
            aggregator = zeros((r.size, len(r.irreps)),mindspore.float32)
            ...
            propagator = (aggregator > 0).clone().to(dtype=mindspore.float32)
            ...
            if not np.allclose(r.change_of_basis, np.eye(r.size)):
                self.register_buffer(f'{name}_change_of_basis', mindspore.Tensor(r.change_of_basis, dtype=mindspore.float32))
                self.register_buffer(f'{name}_change_of_basis_inv', mindspore.Tensor(r.change_of_basis_inv, dtype=mindspore.float32))
            ...
            running_var = ones((1, self._nfields[r.name], len(r.irreps), 1, 1), mindspore.float32)
            running_mean = ones((1, self._nfields[r.name], len(trivials), 1, 1), mindspore.float32)
            self.register_buffer(f'{name}_running_var', running_var)
            self.register_buffer(f'{name}_running_mean', running_mean)
            
            if self.affine:
                weight = Parameter(ones((1, self._nfields[r.name], len(r.irreps), 1, 1), mindspore.float32), requires_grad=True)
                bias = Parameter(ones((1, self._nfields[r.name], len(trivials), 1, 1), mindspore.float32), requires_grad=True)
                self.register_parameter(f'{name}_weight', weight)
                self.register_parameter(f'{name}_bias', bias)
            
        self.register_buffer('num_batches_tracked', mindspore.Tensor(0, dtype=mindspore.int64))
    ...
    def construct(self, input: GeometricTensor) -> GeometricTensor:
        ...
        output = mindspore.numpy.empty_like(input)
        ...
        for name, size in self._sizes:
            ...
            if hasattr(self, f"{name}_change_of_basis_inv"):
                ...
                equation = "ds,bcsxy->bcdxy"
                einsum = mindspore.ops.Einsum(equation)
                slice = einsum((cob_inv, slice))
            ...
            if self.training:
                ...
                running_mean_to_numpy = running_mean.asnumpy()
                name_running_mean = getattr(self, f"{name}_running_mean")
                name_running_mean_to_numpy = name_running_mean.asnumpy()
                running_var_to_numpy = running_mean.asnumpy()
                name_running_var = getattr(self, f"{name}_running_mean")
                name_running_var_to_numpy = name_running_var.asnumpy()
                assert np.allclose(running_mean_to_numpy, name_running_mean_to_numpy)
                assert np.allclose(running_var_to_numpy , name_running_var_to_numpy)
    ...
    def _compute_statistics(self, t: mindspore.Tensor, name: str):
        ...
        equation = "io,bcixy->bcoxy"
        einsum = mindspore.ops.Einsum(equation)
        vars = einsum((vars_aggregator, vars))
    ...
    def _scale(self, t: mindspore.Tensor, scales: mindspore.Tensor, name: str, out: mindspore.Tensor = None):
        
        if out is None:
            out = mindspore.numpy.empty_like(t)
        ...
        equation = "io,bcixy->bcoxy"
        einsum = mindspore.ops.Einsum(equation)
        out[...] = t * einsum((vars_aggregator, scales))
        
        # assert torch.allclose(t, out)
        
        return out
    
    def _shift(self, t: mindspore.Tensor, trivial_bias: mindspore.Tensor, name: str, out: mindspore.Tensor = None):
    
        if out is None:
            out = mindspore.ops.ZerosLike(t)
