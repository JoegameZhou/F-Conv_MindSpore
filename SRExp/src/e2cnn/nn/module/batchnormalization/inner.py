...
class InnerBatchNorm(EquivariantModule):
    ...
    def __init__(self,
        ...
        for s, (contiguous, fields, indices) in grouped_fields.items():
            ...
            if contiguous:
                ...
                _indices[s] = mindspore.Tensor([min(indices), max(indices)+1], mindspore.float32)
            else:
                # otherwise, transform the list of indices into a tensor
                _indices[s] = mindspore.Tensor(indices, mindspore.float32)
        ...
        for s in _indices.keys():
            _batchnorm = BatchNorm3d(
                ...
                1-self.momentum,
                affine=self.affine,
                use_batch_statistics=self.track_running_stats
            )
    ...
    def construct(self, input: GeometricTensor) -> GeometricTensor:
        ...
        output = mindspore.numpy.empty_like(input.tensor)
    ...
    def export(self):
        r"""
        Export this module to a normal mindspore :class:`mindspore.nn.BatchNorm2d` module and set to "eval" mode.
        ...
        batchnorm = mindspore.nn.BatchNorm2d(
            self.in_type.size,
            self.eps,
            1-self.momentum,
            affine=self.affine,
