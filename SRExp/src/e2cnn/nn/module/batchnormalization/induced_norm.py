...
class InducedNormBatchNorm(EquivariantModule):
    ...
    def __init__(self,
        ...
        for id, contiguous in self._contiguous.items():
            if contiguous:
                ...
                _indices[id] = mindspore.Tensor([min(_indices[id]), max(_indices[id])+1], mindspore.int64)
            else:
                # otherwise, transform the list of indices into a tensor
                _indices[id] = mindspore.Tensor(_indices[id], mindspore.int64)
                
            # register the indices tensors as parameters of this module
            self.register_buffer(f'{id}_indices', _indices[id])
            
            ones = mindspore.ops.Ones()
            running_var = ones((1, self._nfields[id], 1, 1, 1, 1), mindspore.float32)
            self.register_buffer(f'{id}_running_var', running_var)
            
            if self.affine:
                weight = Parameter(ones((1, self._nfields[id], 1, 1, 1, 1), mindspore.float32), requires_grad=True)
        ...
        # self.weight = Parameter(mindspore.ops.Ones(len(in_type)), requires_grad=True)
    ...
    def construct(self, input: GeometricTensor) -> GeometricTensor:
        ...
        # n = mindspore.ops.Mul(input.tensor, input.tensor)
