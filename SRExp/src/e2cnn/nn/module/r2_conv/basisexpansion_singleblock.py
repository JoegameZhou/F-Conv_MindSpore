...
class SingleBlockBasisExpansion(BasisExpansion):
    ...
    def __init__(self,
        ...
        sampled_basis = mindspore.Tensor(basis.sample(points)).permute(2, 0, 1, 3)# 这里可以把basis输出来看啦。
#        print(sampled_basis.size())
#        print(sampled_basis)
        ##在这里用matlab存下来看吧？
        ...
        mask = mindspore.Tensor(mask.astype(np.uint8))

        # filter out the basis elements discarded by the filter
        sampled_basis = sampled_basis[mask, ...]
        
        # normalize the basis
        sizes = mindspore.Tensor(sizes, dtype=sampled_basis.dtype)
        ...
        self.sampled_basis = mindspore.Parameter(sampled_basis, requires_grad=False)
    ...
    def construct(self, weights: mindspore.Tensor) -> mindspore.Tensor:
        ...
        return einsum((self.sampled_basis, weights)) #.transpose(1, 2).contiguous() #这是内积，这里不太需要改，
...
def normalize_basis(basis: mindspore.Tensor, sizes: mindspore.Tensor) -> mindspore.Tensor:
    r"""
    ...
    equation = 'bop...,bpq...->boq...'
    einsum = mindspore.ops.Einsum(equation)
    norms = einsum((basis, basis.transpose(1, 2)))
    ...
    equation = 'bii...->b'
    einsum = mindspore.ops.Einsum(equation)
    norms = mindspore.einsum((norms))
    # norms = norms.reshape(b, -1).sum(1)
    norms /= sizes

    norms[norms < 1e-15] = 0
    
    norms = mindspore.ops.Sqrt(norms)
