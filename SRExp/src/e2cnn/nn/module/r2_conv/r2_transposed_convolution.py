...
from mindspore.ops import Conv2DTranspose
...
class R2ConvTransposed(EquivariantModule):
    ...
    def __init__(self,
        ...
        if bias:
            ...
            if trivials > 0:
                ...
                self.expanded_bias = mindspore.Parameter(zeros(out_type.size), requires_grad=False)
                # self.register_buffer("expanded_bias", zeros(out_type.size))
            else:
                self.bias = None
        ...
        self.filter = mindspore.Parameter(zeros((in_type.size, out_type.size, kernel_size, kernel_size), mindspore.float32), requires_grad=False)
        # self.register_buffer("filter", zeros((in_type.size, out_type.size, kernel_size, kernel_size), mindspore.float32))
    ...
    def construct(self, input: GeometricTensor):
        ...
        conv2d_transpose_input = Conv2DTranspose(out_channel = _filter.shape[0],
        ...
        output = conv2d_transpose_input(input.tensor, weight)
    ...
    def export(self):
        ...
        conv = mindspore.nn.ConvTranspose2d(self.in_type.size,
                                            self.out_type.size,
                                            self.kernel_size,
                                            pad_mode='pad',
                                            padding=self.padding,
...
def compute_basis_params(kernel_size: int,
    ...
    if rings is None:
        ...
        rings = mindspore.ops.LinSpace(0, (kernel_size - 1) // 2, n_rings).tolist()
