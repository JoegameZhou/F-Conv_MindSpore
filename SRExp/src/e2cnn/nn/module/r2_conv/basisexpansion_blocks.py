            ...
            for o_repr in out_type._unique_representations:
                ...
                try:
                    ...
                    basis = space.build_kernel_basis(i_repr, o_repr,
                                                     sigma=sigma,
                                                     rings=rings,
                                                     **kwargs) #这个应该是关键，我们要把这个基底选好，基底是固定的，然后和weight做内积，这里用了 GeneralOnR2的子函数
                    
                    
                    block_expansion = block_basisexpansion(basis, points, basis_filter, recompute=recompute)#这个函数可以执行里进行了basis和weight的内积，同时，在这个里面有对basis的实例化
...
#                    self.XQ_block_expansion = block_expansion
        ...
        for io_pair in self._representations_pairs:
            ...
            if self._contiguous[io_pair]:
                ...
                out_indices, in_indices = meshgrid((_out_indices[io_pair[1]], _in_indices[io_pair[0]]))
                ...
                setattr(self, 'in_indices_{}'.format(io_pair), mindspore.Parameter(in_indices, requires_grad=False))
                setattr(self, 'out_indices_{}'.format(io_pair), mindspore.Parameter(out_indices, requires_grad=False))
...
#        _filter = self.XQ_block_expansion(coefficients)  #这里进行了什么运算不知道，但这里是关键
    ...
    def construct(self, weights: mindspore.Tensor) -> mindspore.Tensor:
        """
        ...
        if self._n_pairs == 1:
            ...
            _filter = self._expand_block(weights, io_pair).reshape(out_indices[2], in_indices[2], self.S) #这里是关键
            
        else:
        
            # build the tensor which will contain te filter
            zeros = mindspore.ops.Zeros()
            _filter = zeros((self._output_size, self._input_size, self.S), mindspore.float32)
            ...
            for io_pair in self._representations_pairs:
                ...
                # expand the current subset of basis vectors and set the result in the appropriate place in the filter
                expanded = self._expand_block(weights, io_pair) #这里是关键吧
...
def _retrieve_indices(type: FieldType):
    ...
    for name, indices in _indices.items():
        ...
        _indices[name] = mindspore.Tensor(indices, dtype=mindspore.int64)
