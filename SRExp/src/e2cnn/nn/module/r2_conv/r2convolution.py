...
import mindspore
from mindpsore import Parameter
import numpy as np
import math
...
class R2Conv(EquivariantModule):
    ...
    def __init__(self,
        ...
        During training, in each construct pass the module expands the basis of G-steerable kernels with learned weights
        ...
        if bias:
            ...
            if trivials > 0:
                ...
                bias_expansion = zeros((self.out_type.size, trivials), mindspore.float32)
                ...
                for r in self.out_type:
                    ...
                    for irr in r.irreps:
                        ...
                        if irr.is_trivial():
                            bias_expansion[p:p+r.size, c] = mindspore.Tensor(r.change_of_basis[:, pi])
                            c += 1
                        pi += irr.size
                    p += r.size
                
                self.bias_expansion = mindspore.Parameter(bias_expansion, requires_grad=False)
                # self.register_buffer("bias_expansion", bias_expansion)
                self.bias = Parameter(zeros(trivials, mindspore.float32), requires_grad=True)
                self.expanded_bias = mindspore.Parameter(zeros(out_type.size, mindspore.float32), requires_grad=False)
                # self.register_buffer("expanded_bias", zeros(out_type.size, mindspore.float32))
            else:
                self.bias = None
        ...
        if basisexpansion == 'blocks':
            self._basisexpansion = BlocksBasisExpansion(in_type, out_type,
                                                        ...
                                                        recompute=recompute)#这里都是在ini层面的操作吧
        ...
        zeros = mindspore.ops.Zeros()
        self.weights = Parameter(zeros(self.basisexpansion.dimension(), mindspore.float32), requires_grad=True)
        self.filter = mindspore.Parameter(zeros((out_type.size, in_type.size, kernel_size, kernel_size), mindspore.float32), requires_grad=False)
        # self.register_buffer("filter", zeros((out_type.size, in_type.size, kernel_size, kernel_size), mindspore.float32))
    ...
    def expand_parameters(self) -> Tuple[mindspore.Tensor, mindspore.Tensor]:
        r"""
        ...
        """
        _filter = self.basisexpansion(self.weights) #这里是关键
    ...
    def construct(self, input: GeometricTensor):
        r"""
        ...
        if self.Padding_mode == 'zeros':
            conv2d = Conv2D(out_channel = _filter.shape[0],
                            kernel_size = (_filter.shape[1], _filter.shape[2]),
                            pad_mode="pad",
                            pad=self.Padding,
                            stride=self.stride,
                            dilation=self.dilation,
                            group=self.groups)
            weight = mindspore.Tensor(np.ones([_filter.shape[0],input.shape[1],_filter.shape[1],_filter.shape[2]]),mindspore.float32)
            output = conv2d(input.tensor, weight)
        else:
            conv2d = Conv2D(out_channel = _filter.shape[0],
                            kernel_size = (_filter.shape[1], _filter.shape[2]),
                            stride=self.stride,
                            ...
                            group=self.groups)
            weight = mindspore.Tensor(np.ones([_filter.shape[0],input.shape[1],_filter.shape[1],_filter.shape[2]]),mindspore.float32)
            num3 = self._reversed_Padding_repeated_twice[3]
            num2 = self._reversed_Padding_repeated_twice[2]
            num1 = self._reversed_Padding_repeated_twice[1]
            num0 = self._reversed_Padding_repeated_twice[0]
            self.Padding_mode = self.Padding_mode.capitalize()
            paddings = ((0,0),(0,0),(num3,num2),(num1,num0))
            pad_op = Pad(paddings=paddings, mode=self.Padding_mode)
            tensor = pad_op(input.tensor)
            output = conv2d(tensor, weight)
    ...
    def train(self, mode=True):
        ...
        :attr:`~e2cnn.nn.R2Conv.expanded_bias` such that they are not recomputed at each construct pass.
        ...
        if mode:
            ...
            if _bias is not None:
                self.expanded_bias = mindspore.Parameter(_bias, requires_grad=False)
    ...
    def check_equivariance(self, atol: float = 0.1, rtol: float = 0.1, assertion: bool = True, verbose: bool = True):
        ...
        x = GeometricTensor(mindspore.Tensor(x, dtype=mindspore.float32), self.in_type)
        
        def shrink(t: GeometricTensor, s) -> GeometricTensor:
            return GeometricTensor(mindspore.Tensor((block_reduce(t.tensor.detach().numpy(), s, func=np.mean)), dtype=mindspore.float32), t.type)
    ...
    def export(self):
        ...
        conv = mindspore.nn.Conv2d(self.in_type.size,
                               self.out_type.size,
                               self.kernel_size,
                               pad_mode = 'pad',
...
def compute_basis_params(kernel_size: int,
    ...
    if rings is None:
        ...
        rings = mindspore.ops.LinSpace(0, (kernel_size - 1) // 2, n_rings) * dilation
        rings = rings.tolist()
