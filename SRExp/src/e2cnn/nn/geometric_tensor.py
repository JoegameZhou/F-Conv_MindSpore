...
class GeometricTensor:
    ...
    def __init__(self, tensor: Tensor, type: FieldType):
        ...
        It is wrapping a common :class:`mindspore.Tensor` and endows it with a (compatible) :class:`~e2cnn.nn.FieldType` as
        *transformation law*.
        ...
        For example::
            ...
            t1 = e2cnn.nn.GeometricTensor(mindspore.ops.StandardNormal(1, 24, 3, 3), type)
            t2 = e2cnn.nn.GeometricTensor(mindspore.ops.StandardNormal(1, 24, 3, 3), type)
        ...
        We give few examples to illustrate this behavior::
            ...
            geom_tensor = e2cnn.nn.GeometricTensor(mindspore.ops.StandardNormal(10, type.size, 9, 9), type)
            
            geom_tensor.shape
            >> torch.Size([10, 15, 9, 9])#difference
            
            geom_tensor[1:3, :, 2:5, 2:5].shape
            >> torch.Size([2, 15, 3, 3])#difference
            
            geom_tensor[..., 2:5].shape
            >> torch.Size([10, 15, 9, 3])#difference
            ...
            >> torch.Size([10, 8, 9, 9])#difference
            ...
            >> torch.Size([10, 7, 9, 9])#difference
        ...
        Args:
            tensor (mindspore.Tensor): the tensor data
    ...
    def split(self, breaks: List[int]):
        ...
        Example ::
            ...
            geom_tensor = e2cnn.nn.GeometricTensor(mindspore.ops.StandardNormal(10, type.size, 7, 7), type)
            
            geom_tensor.shape
            >> torch.Size([10, 15, 7, 7])#difference
    ...
    def transform_fibers(self, element) -> 'GeometricTensor':
        ...
        """
        rho = mindspore.Tensor(self.type.representation(element), dtype=float32)
        equation = "oi,bihw->bohw"
        einsum = mindspore.ops.Einsum(equation)
        data = einsum((rho, self.tensor.contiguous())).contiguous()
    ...
    def to(self, *args, **kwargs):
        ...
        Applies :meth:`torch.Tensor.to` to the underlying tensor and wraps the resulting tensor in a new#difference
    ...
    def __mul__(self, other: Union[float, mindspore.Tensor]) -> 'GeometricTensor':
        r"""
        Scalar product of this :class:`~e2cnn.nn.GeometricTensor` with a scalar.
        The operation is done inplace.
        
        The scalar can be a float number of a :class:`mindspore.Tensor` containing only
        one scalar (i.e. :func:`mindspore.ops.Size` should return `1`).
    ...
    def __imul__(self, other: Union[float, mindspore.Tensor]) -> 'GeometricTensor':
        r"""
        Scalar product of this :class:`~e2cnn.nn.GeometricTensor` with a scalar.
...
def tensor_directsum(tensors: List['GeometricTensor']) -> 'GeometricTensor':
    ...
    data = mindspore.ops.Concat(1)([t.tensor for t in tensors])
